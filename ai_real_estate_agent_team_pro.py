...
if __name__ == "__main__":
    main()
t['affiliation_tab']}</h2>", unsafe_allow_html=True)

        st.markdown("""
        <div style="background: rgba(255, 255, 255, 0.95); border: 3px solid #D4AF37; 
                    border-radius: 28px; padding: 40px; margin-bottom: 40px;
                    box-shadow: 0 12px 40px rgba(212, 175, 55, 0.2);">
            <h3 style="color: #D4AF37; font-family: Playfair Display, serif; font-size: 28px; margin-bottom: 16px;">
                💎 Premium Tools for Discerning Investors
            </h3>
        ...
        for key, prog in AFFILIATE_LINKS.items():
            with st.container():
                st.markdown(f"""
                <div class="property-card" style="margin: 20px 0; border-left: 5px solid #D4AF37;">
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                        <div style="flex: 1; min-width: 280px;">
                            <h4 style="color: #D4AF37; font-family: Playfair Display, serif; font-size: 26px; margin-bottom: 12px;">
                                {prog['name']}
                            </h4>
                            <p style="color: #0369A1; font-size: 17px; line-height: 1.6;">
                                {prog['description'].get(lang, prog['description']['en'])}
                            </p>
                            <p style="color: #0284C7; font-size: 15px; margin-top: 12px; font-weight: 600;">
                                💰 {t['commission']}: {prog['commission']} | 📂 {t['category']}: {prog['category'].get(lang, prog['category']['en'])}
                            </p>
                        </div>
                        <div style="margin-left: 32px; margin-top: 16px;">
                            <a href="{prog['url']}" target="_blank" 
                               style="background: linear-gradient(135deg, #D4AF37 0%, #F0E68C 100%); 
                                      color: #0C4A6E; padding: 14px 28px; border-radius: 14px; 
                                      text-decoration: none; font-weight: 700; font-size: 16px;
                                      display: inline-block; box-shadow: 0 6px 20px rgba(212, 175, 55, 0.4);">
                                🔗 Access
                            </a>
                        </div>
                    </div>
    ...
    <div class="footer">
        ...
        <p style="color: #0284C7; font-size: 16px; margin-top: 12px;">
            🌍 Global Markets | 🏛️ Premium Properties | 💎 Exclusive Service
        </p>
    </div>
    """, unsafe_allow_html=True)
